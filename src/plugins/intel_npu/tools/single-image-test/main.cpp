...
DEFINE_bool(random, false, "Run on rundom input)");
DEFINE_string(
    ref_dir,
    "",
...
void parseCommandLine(int argc, char* argv[]) {
    ...
    std::cout << "    Random input:                             " << FLAGS_random << std::endl;
...
ov::Tensor generateRandom(const ov::element::Type& precision, const ov::Shape& shape) {
    const ov::Tensor tensor(precision, shape);
    // TODO: populate the tenor with random value
    return tensor;
}
    ...
    try {
        ...
        for (size_t numberOfTestCase = 0; numberOfTestCase < inputFilesPerCase.size(); ++numberOfTestCase) {
            ...
            for (const auto& inputInfo : inputsInfo) {
                ...
                ov::Tensor tensor;
                if (FLAGS_random == false) {
                    std::cout << "Case load input" << std::endl;
                    tensor =
                        !FLAGS_img_as_bin
                    ...
                    std::cout << "Case random input" << std::endl;
                    tensor = generateRandom(precision, dataShape);
                }
